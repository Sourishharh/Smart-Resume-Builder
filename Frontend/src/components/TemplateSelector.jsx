...
const TemplateSelector = ({selectedTemplate, onChange}) => {
    ...
    const templates=[
        {
        ...
        },
         {
            id: "modern",
            name: "Modern",
            preview:"Sleek design with stratigic use of color and modern font choices"
        },
         {
            id: "minimal-image",
            name: "Minimal Image",
  ...
  return (
    <div className='relative'>
       <button onClick={()=> setIsOpen(!isOpen)} className='flex items-center gap-1 text-sm text-blue-600 bg-gradient-to-br from-blue-50 to-blue-100 ring-blue-300 hober:ring transition-all px-3 py-2 rounded-lg'>
              ...
              {templates.map((template)=>(
                <div key={template.id} onClick={()=>{onChange(template.id); setIsOpen(false)}} className={`relative p-3 border rounded-md cursor-pointer transition-all ${selectedTemplate === template.id ? "border-blue-400 bg-blue-100": "border-gray-300 hover:border-gray-400 hover:bg-gray-100"}`}>
                   ...
                   <div className='space-y-1'>
                     ...
                     <div className='mt-2 p-2 ng-blue-50 rounded text-xs text-gray-500 italic'>{template.preview}</div>
                   </div>
                </div>
              ))}
         </div>
